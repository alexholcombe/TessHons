...
trialsPerCondition = 100 #tesschange from 23, patriciachange from Tess's 17
defaultNoiseLevel = 0
if not demo:
    allowGUI = False
...
def calcAndPredrawStimuli(stimList,spacing,i,j,k):
   ...
   indices = [i,j,k]
   textStimuli = list()
   for i in range(3): #Create  stimuli, one for each stream
...
if doStaircase:
    ...
    stepSizesLinear = [.6,.6,.5,.5,.4,.3,.3,.1,.1,.1,.1,.05] #for pratice [.001]
    minVal = bgColor[0]+.15
...
while nDoneMain < trials.nTotal and expStop!=True: #MAIN EXPERIMENT LOOP
    ...
    idxsStream1 = [0,1]; idxsStream2 = [0,1]
