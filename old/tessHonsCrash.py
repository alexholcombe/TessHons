...
if sys.platform == "win32":  #this means running in PSYC computer labs
    pathToData = 'dataRaw'  # os.path.join('..',"Submissions")
else:
    pathToData = 'dataRaw'
...
def calcAndPredrawStimuli(stimList,i,j):
   ...
   textStimuli = list()
   for idx in range(2):
        stim = visual.TextStim(myWin, text = stimList[indices[idx]],
...
if experiment['stimType']=='trigram':
...
def oneFrameOfStim( n,cue,seq1,seq2,seq3,cueDurFrames,letterDurFrames,thisTrial,textStimuliStream1,textStimuliStream2,
...
def do_RSVP_stim(thisTrial, seq1, seq2, seq3, ltrColorThis, proportnNoise,trialN,thisProbe):
    ...
    for n in range(trialDurFrames): #this is the loop for this trial's stimulus!
            worked = oneFrameOfStim( n,cue,seq1,seq2,seq3,cueDurFrames,letterDurFrames,thisTrial,stimuliStream1,stimuliStream2,
...
while nDoneMain < trials.nTotal and expStop!=True: #MAIN EXPERIMENT LOOP
    ...
    ts  =  do_RSVP_stim(thisTrial, idxsStream1, idxsStream2, ltrColorThis, noisePercent/100.,nDoneMain,thisProbe)
