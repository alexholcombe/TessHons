...
autopilot=False
demo=False #False
exportImages= True #quits after one trial
...
#add letters vertical arranged, rotated right (6) and rotated left (7)
...
def calcAndPredrawStimuli(stimList,i,j,k):
   ...
   textStimuli = list()
   for i in range(3):
        stim = stimList[indices[i]]
        if checkAlignment:
            stim = 's'
        stim = visual.TextStim(myWin, text = stim,
...
for i in range( experiment['numSimultaneousStim'] ): 
...
def do_RSVP_stim(thisTrial, seq1, seq2, seq3, ltrColorThis, proportnNoise,trialN,thisProbe):
    ...
    for n in range(trialDurFrames): #this is the loop for this trial's stimulus!
            ...
            if exportImages:
                ...
                myWin.saveMovieFrames('images_sounds_movies/frame' + str(framesSaved) + '.png') #mov not currently supported 
                framesSaved +=1
            myWin.flip()
...
while nDoneMain < trials.nTotal and expStop!=True: #MAIN EXPERIMENT LOOP
    ...
    trial = trials.__next__() # trials.next()
    ...
    if nDoneMain==0: #First trial
        ...
        if doStaircase:
            ...
            ltrColorThis = staircase.__next__()
            #if ltrColorThis <= 1:
            #    howManyMoreFrames = 0
    ...
    if thisProbe == 'both': #Either have word on both sides or letter on both sides
        ...
        responseOrder = list( range(numToReport) ) #make sure it's a list rather than an iterator so you can subscript it later
...
if exportImages:
    print('program terminating because exportImages == TRUE')
else:
